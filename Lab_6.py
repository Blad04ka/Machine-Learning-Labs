...
print(
    f'Процент наблюдений, которые кластеризовать не удалось: {list(clustering.labels_).count(-1) / len(list(clustering.labels_))})
...
for metric in optic_metrics:
    ...
    print( f'при max_eps=2 и min_samples=3 и metric={metric} количество кластеров ровняется: {len(set(clustering.labels_)) - 1})
